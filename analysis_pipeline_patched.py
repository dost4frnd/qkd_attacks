...
def save_confusion_matrix(cm, labels, outpath, title):
    ...
    ax.set_ylabel('True label'); ax.set_xlabel('Predicted label'); fig.tight_layout(); fig.savefig(outpath, dpi=200, bbox_inches='tight'); plt.close(fig)
def plot_feature_drift(long_df: pd.DataFrame, outdir: Path):
    ...
    grouped = long_df.groupby(['label', 't'])[features].mean().reset_index(); labels = grouped['label'].unique().tolist()
    for feat in features:
        fig, ax = plt.subplots(figsize=(10,5))
        for label in labels:
            tmp = grouped[grouped['label']==label]; ax.plot(tmp['t'], tmp[feat], label=label)
        ax.set_title(f'Feature drift over time: {feat}'); ax.set_xlabel('timestep'); ax.set_ylabel(feat); ax.legend(fontsize=8, ncol=2); ax.grid(alpha=0.3); fig.tight_layout(); fig.savefig(outdir / f'drift_{feat}.png', dpi=200, bbox_inches='tight'); plt.close(fig)
...
def plot_metrics(metrics_df, outpath):
    ...
    ax.set_xticks(x); ax.set_xticklabels(metrics_df['model'].tolist(), rotation=30, ha='right'); ax.set_ylim(0,1.05); ax.legend(); ax.set_title('Model metrics comparison'); fig.tight_layout(); fig.savefig(outpath, dpi=200, bbox_inches='tight'); plt.close(fig)
...
def plot_attention(attn, outpath, title='Transformer attention'):
    fig, ax = plt.subplots(figsize=(7,6)); im=ax.imshow(attn, aspect='auto'); fig.colorbar(im, ax=ax); ax.set_title(title); ax.set_xlabel('Key timestep'); ax.set_ylabel('Query timestep'); fig.tight_layout(); fig.savefig(outpath, dpi=220, bbox_inches='tight'); plt.close(fig)
@torch.no_grad()
...
def run_one_dataset(data_path: Path, run_dir: Path, outdir: Path, seed=42):
    ...
    # Map labels safely: known classes keep their encoder id, unseen labels become -1.
    # This avoids LabelEncoder.transform() failures when unknown_attack is present.
    y = np.full(len(labels_str), -1, dtype=int)
    for i, lbl in enumerate(labels_str):
        if lbl in known_classes:
            y[i] = int(label_encoder.transform([lbl])[0])
    ...
    for name, model in [('qlstm', qlstm), ('lstm', lstm), ('transformer', transformer)]:
        ...
        try:
            if known_mask.any() and len(np.unique(y_true[known_mask])) > 1:
                metrics['roc_auc_ovr_macro'] = roc_auc_score(y_true[known_mask], probs[known_mask], multi_class='ovr', average='macro')
            else:
        ...
        known_mask = y_true != -1
        if known_mask.any():
            save_confusion_matrix(confusion_matrix(y_true[known_mask], y_pred[known_mask], labels=list(range(len(label_encoder.classes_)))), label_encoder.classes_, outdir / f'{name}_confusion_matrix.png', f'{name} confusion matrix')
        else:
            save_confusion_matrix(confusion_matrix(y_true, y_pred), label_encoder.classes_, outdir / f'{name}_confusion_matrix.png', f'{name} confusion matrix')
        ...
        if known_mask.any():
            ...
            print('No known-label samples available for supervised reporting in this split.')
    ...
    if loaded['autoencoder']:
        ...
        fig, ax = plt.subplots(figsize=(8,5)); ax.hist(test_err, bins=40, alpha=0.8); ax.axvline(threshold, linestyle='--'); ax.set_title('Autoencoder reconstruction error distribution'); ax.set_xlabel('MSE'); ax.set_ylabel('count'); fig.tight_layout(); fig.savefig(outdir / 'autoencoder_scores_hist.png', dpi=200, bbox_inches='tight'); plt.close(fig)
    ...
    plot_metrics(metrics_df, outdir / 'metrics_comparison.png')
    # drift plots
    ...
    if long_path.exists():
        ...
        plot_feature_drift(long_df, outdir)
    # t-SNE
    if 'transformer' in preds:
        _, _, _, feats = preds['transformer']
        plot_tsne(feats, y[test_idx], label_encoder.classes_, outdir / 'tsne_transformer_embeddings.png', 't-SNE of transformer embeddings')
    # attention
    if loaded['transformer']:
        ...
        plot_attention(attn, outdir / 'transformer_attention.png', 'Average transformer attention')
...
def compare_runs(run_dirs: List[Path], outdir: Path):
    ...
    for col in cols:
        ...
        ax.set_title(f'Cross-domain robustness: {col}'); ax.set_ylabel(col); ax.tick_params(axis='x', rotation=30); ax.legend(fontsize=8); ax.grid(alpha=0.3); fig.tight_layout(); fig.savefig(outdir / f'cross_domain_{col}.png', dpi=200, bbox_inches='tight'); plt.close(fig)
    fig, ax = plt.subplots(figsize=(12,6))
    for model in models:
        tmp = allm[allm['model']==model]
        if 'f1_macro' in tmp.columns: ax.plot(tmp['run_dir'], tmp['f1_macro'], marker='o', label=model)
    ax.set_title('Cross-domain F1 macro comparison'); ax.set_ylabel('F1 macro'); ax.tick_params(axis='x', rotation=30); ax.legend(fontsize=8); ax.grid(alpha=0.3); fig.tight_layout(); fig.savefig(outdir / 'cross_domain_f1_macro.png', dpi=200, bbox_inches='tight'); plt.close(fig)
def main():
